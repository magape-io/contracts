...
contract CIDDB {
    ...
    function stringData(string memory str) external returns (uint256 nwc) {
        assembly {
            // if(fet[str] > 0)
            ...
            for {
                let i := 0x01
            } lt(mul(0x20, sub(i, 0x01)), mload(str)) {
                i := add(i, 0x01)
            } {
    ...
    function stringData(uint256 cid) external view returns (string memory) {
        assembly {
            // str(length)
            mstore(0x00, cid)
            mstore(0x20, CDB)
            ...
            for {
                let i := 0x01
            } lt(mul(0x20, sub(i, 0x01)), sload(ptr)) {
                i := add(i, 0x01)
            } {
